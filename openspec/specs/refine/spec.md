...
**R5.** `refine_save_iteration` MUST insertar una fila en `refinements` con `thread_id`, `iteration_number`, `requirements`, `instructions` (nullable), `output` y `status = 'in_progress'`; `iteration_number` MUST ser `MAX(iteration_number) + 1` calculado dentro de una transacción. Si el hilo está en `status = 'completed'`, MUST reabrirlo primero (todas las filas → `'in_progress'`) dentro de la misma transacción.
...
**R8.** `refine_finalize` MUST actualizar `status = 'completed'` en todas las filas del `thread_id` indicado.

**R9.** Después de `refine_finalize`, cualquier llamada a `refine_save_iteration` sobre ese `thread_id` MUST reabrirlo automáticamente (sin error ni warning); la reapertura es transparente para el caller.

**R10.** Cada fila en `refinements` MUST persistir el campo `instructions` junto con `output` (puede ser `NULL` si no se proveyeron instrucciones).

**R11.** Las cuatro tools nuevas MUST registrarse en el array `tools[]` de `createRefineSkill` y MUST ser enrutadas desde `execute` con `switch` por nombre.

**R12.** El CLI MUST exponer el grupo `jarvis refine` con los subcomandos `save`, `iterate`, `list`, `show` y `finalize`; todos los mensajes al usuario MUST estar en español.

**R13.** `refine_requirements` sin `thread_id` MUST generar un UUID v4 y retornar un header meta `<!-- refine:meta thread_id: <uuid> iteration: 1 -->` al inicio del prompt; NO MUST persistir nada en la base de datos.

**R14.** `refine_requirements` con `thread_id` NO MUST incluir `### Input Requirements` en el body del prompt iterativo; solo `### Previous Output`, `### Correction Instructions`, `### Refinement Instructions` según corresponda.
...
- Then: se inserta una fila con `iteration_number = 1`, `status = 'in_progress'`, `instructions = NULL`
...
**E5 — Reapertura automática al guardar sobre hilo finalizado**
- Given: `thread_id = "xyz"` está en `status = 'completed'` (todas sus filas)
- When: el caller llama a `refine_save_iteration(thread_id: "xyz", ...)`
- Then: el tool reabre el hilo (todas las filas → `'in_progress'`), inserta la nueva iteración con `status = 'in_progress'`, y retorna la fila recién insertada sin error ni warning

**E6 — refine_requirements sobre hilo completado sin warning**
- Given: `thread_id = "xyz"` está en `status = 'completed'`
- When: el caller llama a `refine_requirements(requirements, thread_id: "xyz")`
- Then: el prompt se construye normalmente (usando el último output como contexto), SIN incluir ninguna advertencia. Si el caller persiste una nueva iteración, el hilo se reabre automáticamente.
...
**E9 — One-shot genera thread_id sin persistencia**
- Given: no existe ninguna fila para un nuevo hilo
- When: el caller llama a `refine_requirements(requirements)` sin `thread_id`
- Then: el tool genera un UUID v4, construye header `<!-- refine:meta thread_id: <uuid> iteration: 1 -->`, retorna `header + prompt`, y NO persiste nada en la base de datos

**E10 — Path iterativo sin `### Input Requirements`**
- Given: existe un hilo con `thread_id = "abc"` y al menos una iteración persistida
- When: el caller llama a `refine_requirements(requirements, thread_id: "abc", instructions: "revisión")`
- Then: el body del prompt contiene `### Previous Output` (último output del hilo), `### Correction Instructions` ("revisión"), y `### Refinement Instructions`, pero NO `### Input Requirements`

**E11 — CLI jarvis refine iterate sin --thread**
- Given: el usuario ejecuta `jarvis refine iterate --input /ruta/req.txt` sin especificar `--thread`
- When: el CLI completa la llamada
- Then: el CLI invoca `refine_requirements` sin `thread_id`, extrae el `thread_id` del header de la respuesta, imprime el prompt y el UUID extraído para que el usuario pueda copiarlo

**E12 — Concurrencia: iteration_number monotónico**
...
- `thread_id` vacío string (`""`) MUST tratarse igual que `thread_id` ausente (genera UUID one-shot).
...
- El repo no expone `DELETE`; las filas en `refinements` son inmutables una vez insertadas (excepto el campo `status`, que transiciona `in_progress` ↔ `completed` en operaciones de reapertura y finalización).
- Reapertura es transaccional: todas las filas del hilo transicionan `completed` → `in_progress` atomically, junto con el insert de la nueva iteración.
